...
export default function useWebRTC({ onRoomFull, onJoined, onLeave }) {
  ...
  useEffect(() => {
    if (!socketRef.current) {
      ...
      socketRef.current.on('peer-joined', (pid) => {
        ...
        createOffer(pid); // Only the second peer creates the offer
      });
  ...
  const leaveRoom = () => {
    ...
    closePeerConnection();
    if (onLeave) onLeave();
    // Optionally, socketRef.current.leave(roomId) if server supports
  };
